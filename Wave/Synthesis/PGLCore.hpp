...
void reDisplay(void) {
	if (isRunning) {
		...
		if (flag < 1) {
			PGLVector a;
			tmp[0] = atom[0];
			a = -(tmp[0].p.y - 100) * 5 * PGLPoint(0, 1) / atom[0].m;
			tmp[0].v = tmp[0].v + timeStep * a;
			tmp[0].p = tmp[0].p + timeStep * tmp[0].v;
			if (fabs(tmp[0].p.y - 100) <= 1) {
				flag++;
				tmp[0].p.y = 100;
			}
			tmp[nAtom - 1] = atom[nAtom - 1];
			tmp[nAtom - 1].v = tmp[0].v;
			tmp[nAtom - 1].p.y = tmp[0].p.y;
		} else {
			tmp[0] = atom[0];
			tmp[0].p.y = 100;
		}
